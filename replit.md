...
-   **Smart Dock & Launcher**: A compact, rounded dock at the bottom provides quick access to favorite widgets. A searchable launcher panel, accessible via a "More" button or keyboard shortcut (Ctrl/Cmd+K), lists all widgets with real-time filtering, quick access to favorites, and recently used widgets. All widget names are localized in Swedish.
